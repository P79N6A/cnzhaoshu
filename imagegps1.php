...
<body>
    ...
    <div id="workitems" class="weui-cells" style="padding: 10px;">
        <!-- <div class="weui-cell"> -->
...
// 必然登录, userid=22&mapid=10&maporderid=15&active=2  
// var mapid = urlRequest('mapid');
var maporderid = urlRequest('maporderid');
var user = getcookie('user2'); 
    user = JSON.parse(user);  // {userid,role,projectid}
    // user = {userid:811, name:"杨三木", phone:"13801305541"};

var mapid, record, localIds, localIdsIndex, gps, uploadIndex, uploadData;
...
// var dot_workitem = doT.template(document.getElementById('dot_workitem').text);
// $('#workitem').html(dot_workitem(dicttionary_work));
            
// 检查权限，获取当前养护项目
function checkTree(maporderid) {
    record = null;
    $.getJSON('com/record/check.php', {maporderid: maporderid}, function(data) {
        console.log(data);
        ...
        if (data) {
            ...
            $('#treename').html(tree.name + ' ' + tree.number + tree.unit);
            $('#treeattribute').html(tree.attribute);
...
function setRecord(data) {
    if (data) {
        ...
        if (record.type==8) $('#otherwork').val(record.active);
...
$("#workitems input").on('click', function() { 
    ...
    if (!record || record.type!=worktype) {
        checkRecord(maporderid, worktype);
    }
}); 

function checkRecord(maporderid, type) {
    $('#uploaderFiles').html('');
    record = null;

    // if (type==8) type = $('#otherwork').val();

    $.getJSON('com/record/check.php', {maporderid: maporderid, type: type}, function(data) {
        setRecord(data);
    });    
}
    ...
    if (uploadIndex < $uploaderInput[0].files.length) {
        ...
        // if (file.type.indexOf('image') == 0) {
            lrz(file, {
                ...
                rst.formData.append('mapid', mapid);
                rst.formData.append('maporderid', maporderid);
...
$(function(){
    ...
    function uploadImage(i) {
        wx.uploadImage({
            ...
            success: function (res) {
                // 服务器端处理
                $.post('com/record/upload_wechat.php',
                    {
