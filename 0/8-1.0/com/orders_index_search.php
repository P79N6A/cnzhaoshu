<?php 
	...
	if($search){
		$sql = 'select id,ordername,address,time from tree_order_index where tendering = 2 and userid=? '.$search.'order by expiration_date desc';
		
		$result = $db->prepare_query($sql, $searchkey);
		echo json_encode($result);
	}else{
		$sql = 'select id,ordername,address,time from tree_order_index where tendering = 2 and userid=? order by expiration_date desc limit ?,?';
