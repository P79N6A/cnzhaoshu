...
<head>
    ...
    <style type="text/css">
        ...
        .button_paydown,.button_payfull,.button_applyrefund,.button_detail,.button_evaluate,.button_receipt,.button_refunding,.button_notreceipt,.button_handing,.button_over,.button_retreat,.button_notretreat,.button_applydeposit,.button_agreepay,.button_disagreepay,.button_delivery{
            float: right;
            display: inline-block;
            padding: .15em .4em;
            min-width: 8px;
...
<body ontouchstart>
        ...
        {{ for(var i in it) { }}
            <div class="buy_onedatainfo" treeuserid="{{=it[i].treeuserid}}" order_oneid="{{=it[i].id}}">
                <div>
                    ...
                    <div class="{{=(it[i].order_switch == 5) ? '' : 'hide'}}">提醒：乙方申请定金</div>
                </div>
                <div>
                    <div class="button_detail">查看进程</div>                    
                    {{=(it[i].is_evaluate) ? '' : '<div class="button_evaluate">评价</div>'}}
                    {{=((it[i].state == 2) && (it[i].order_switch == 1)) ? '<div class="button_receipt">确认收货</div>' : ''}}
                    {{=((it[i].state == 2) && (it[i].order_switch == 1)) ? '<div class="button_notreceipt">拒收货</div>' : ''}}
                    {{=((it[i].order_switch == 3) || (it[i].order_switch == 2)) ? '<div class="button_handing">正在协商</div>' : ''}}
                    {{=((it[i].deposit_switch == 1) && (it[i].order_switch == 1) && (it[i].state == 1)) ? '<div class="button_applyrefund">申请退定金</div>' : ''}}
                    {{=((it[i].state < 2) && (it[i].order_switch == 1)) ? '<div class="button_payfull ">付全款</div>' : ''}}
                    {{=(it[i].state < 1) ? '<div class="button_paydown">付定金</div>' : ''}}
                    {{=(it[i].state == 4) ? '<div class="button_over">交易完成</div>' : ''}}
        ...
        {{ for(var i in it) { }}
            <div class="sell_onedatainfo" userid="{{=it[i].userid}}" order_oneid="{{=it[i].id}}">
                ...
                <div>
                    ...
                    {{=((it[i].deposit_switch == 1) && (it[i].order_switch == 5)) ? '<div class="button_over">等待对方同意</div>' : ''}}
                    {{=(it[i].order_switch == 2) ? '<div class="button_retreat">退定金</div>' : ''}}
                    {{=(it[i].order_switch == 2) ? '<div class="button_notretreat">不退定金</div>' : ''}}
                    {{=(it[i].order_switch == 3) ? '<div class="button_handing">正在协商</div>' : ''}}
                    {{=(it[i].deposit_refund_time) ? '<div class="button_delivery">发货</div>' : ''}}
                    {{=(it[i].state == 4) ? '<div class="button_over">交易完成</div>' : ''}}
                </div>
            </div>
        {{ } }}
            ...
            $('#screenlist').on('click','.besupplier',function(){
                ...
                $.getJSON('/com/tobe_mysupplier.php',{userid:user.userid,supplierid:supplierid,state:'null'},function(data){
                    if(data){
                        ...
                        that.html('中标');
                        that.parent().attr('state','1');
                        that.addClass('bid');
                        that.removeClass('besupplier'); 
    ...
    <script type="text/html" id="tpl_bidhistory">
        ...
        <script type="text/javascript">            
            ...
            $('.showstar').click(function(){
                ...
                switch(starsn){
                    case 1:
                        $('.star1').addClass('starover');
                        starnumber = 1;
                        break;
                    case 2:
                        $('.star1').addClass('starover');
                        $('.star2').addClass('starover');
                        starnumber = 2;         
                        break;
                    case 3:
                        $('.star1').addClass('starover');
                        $('.star2').addClass('starover');
                        $('.star3').addClass('starover');
                        starnumber = 3;     
                        break;
                    case 4:
                        $('.star1').addClass('starover');
    ...
    <script type="text/html" id="tpl_managerbuy">
        ...
        <script type="text/javascript">
            var timearray = ['refund_time','payment_time','order_switch_time','button_receipt_time','delivery_time','deposit_refund_time','fullamount_time','deposit_time','orderstart_time'];
            var timeimpact = ['refund_mount','payment_mount',' ',' ',' ','deposit','fullamount','deposit',' '];
            var timename = ['退还资金','已完成交易支付对方','申请退还资金','已收货','卖家已发货','对方已收定金','已付全款','已付定金','下订单'];
            ...
            $('#orderinfo').on('click','.button_receipt',function(){
                ...
                if(!sure) return;
                var order_oneid = that.parents('.buy_onedatainfo').attr('order_oneid');
                var userid = that.parents('.buy_onedatainfo').attr('buy_onedatainfo');
                $.get('/com/order_one_receipt.php',{id: order_oneid,userid:userid},function(result){
                    if(result){
                        ...
                        for (var i = 0; i < buy_alldata.length; i++) {
                            if(buy_alldata[i].id == order_oneid){
                                buy_alldata[i].button_receipt_time =  result;        
                                buy_alldata[i].state =  3;
                            }
                            if(buy_alldata[i].state <3){
                                buy_unfinsheddata[buy_unfinsheddata.length] = buy_alldata[i];
                            }else{
                                buy_finsheddata[buy_finsheddata.length] = buy_alldata[i];
                            }                
                ...
                if($('.uploads').hasClass('ar')){
                    ...
                    $.get('/com/order_one_freeze.php',{serial_number:serial_number,remark:remark},function(result){
                        if(result){
                            for (var i = 0; i < buy_unfinsheddata.length; i++) {
                                if(order_oneid == buy_unfinsheddata[i].id){           
                                    buy_unfinsheddata[i].order_switch =  3;            
                                    buy_unfinsheddata[i].order_switch_remark =  remark;
                                    buy_unfinsheddata[i].order_switch_time =  result;             
                                }                
                            }
    ...
    <script type="text/html" id="tpl_managersell">
        ...
        <script type="text/javascript">
            var timearray = ['payment_time','refund_time','order_switch_time','receipt_time','delivery_time','deposit_refund_time','fullamount_time','deposit_time','orderstart_time'];
            var timeimpact = ['payment_mount','refund_mount',' ',' ',' ','deposit','fullamount','deposit',' '];
            var timename = ['已完成交易收到资金','退还资金','申请退还资金','买家确认收货','发货','收到定金','买家已付全款','买家已付定金','买家下订单'];
                    ...
                    if(result){
                        ...
                        for (var i = 0; i < sell_alldata.length; i++) {
                            if(sell_alldata[i].id == order_oneid){
                                ...
                                sell_alldata[i].order_switch =  4;
                                sell_alldata[i].state =  4;
            ...
            $('#orderinfo').on('click','.button_delivery',function(){
                ...
                $.get('/com/order_onedelivery.php', {id:order_oneid}, function(result) {
                    if(result){
                        ...
                        for (var i = 0; i < sell_alldata.length; i++) {
                            if(sell_alldata[i].id == order_oneid){
                                sell_alldata[i].delivery_time = result;
                            }
                            if(sell_alldata[i].state != 4){
                                sell_unfinsheddata[sell_unfinsheddata.length] = sell_alldata[i];
                            }else{
