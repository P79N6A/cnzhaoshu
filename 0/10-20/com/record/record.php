...
class record 
{
	...
	*/
	...
	public static function delete($recordid, $treeid) 
	{
		$db = new db();
		...
		// 检查并删除二维码
		$tree = $db->prepare_query('select qrcode,photo from map_tree where id=? and qrcode is not null', array($treeid));
		if ($tree) {
			$tree = $tree[0];
