...
$db = new db();
$sql = 'select serial_number from order_one where id=?';
$serial_number = $db->prepare_query($sql,array($order_oneid))[0]['serial_number'];
...
$input->SetBody($name);
$input->SetOut_trade_no($serial_number);
$input->SetTotal_fee($money);
$input->SetTime_start(date("YmdHis"));
$input->SetTime_expire(date("YmdHis", time() + 600));
$input->SetNotify_url("http://cnzhaoshu.com/wxpay/example/dp_notify.php");
			...
			WeixinJSBridge.invoke(
				...
				function(res){
					...
					if(res.err_msg == 'get_brand_wcpay_request:ok'){
						// alert('支付成功');
						window.location.href="../../yusuanphone.php#managerbuy";
