...
<body>
    ...
    <script type="text/javascript">
		...
		function payfail(gift){
			$.getJSON('/com/deleteorderpay.php',{serial_number:serial_number},function(json){
				if(json){
					alert('支付失败!');
					history.go(-2);
				} 
		...
		function result(serial_number){
			$.getJSON('/com/update_orderstate.php',{serial_number:serial_number,bid_userid:bid_userid,userid:user.userid,money:money,tree_order_id:tree_order_id},function(json){
				if(json) alert('支付成功！');
				history.go(-2);
			})
		}
