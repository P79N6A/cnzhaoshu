...
<body>
    ...
    <div id="attributein">
        ...
        <div class="buttons">
            <a class="weui-btn weui-btn_mini weui-btn_primary reset">取消</a>
            <a class="weui-btn weui-btn_mini weui-btn_primary add">修改</a>
        </div>
    </div>
    <script id="dot_onedata" type="text/x-dot-template">
        {{ for(var i in it) { }}
            <div class="onedata" data-id="{{=it[i].id}}" data-orderid="{{=it[i].orderid}}" data-username="{{=it[i].username}}">
                ...
                <div class="row4 {{= (it[i].phone) ? 'ismanager' : 'notmanager'}}">
                    <div class="row41">{{= (it[i].phone) ? '手机号码：'+it[i].phone : ''}}</div>
    ...
    <script id="dot_thisdata" type="text/x-dot-template">
        <div class="row3"><div class="row31">编号: {{=it.id}}</div><div class="row32">{{=string2shortTime(it.time)}}</div></div>
        <div class="row1">
            <div class="onedatainfo_name">{{=it.name}}</div>
            <div class="onedatainfo_number">{{=it.count ? it.count : ''}}{{=it.unit ? '('+it.unit+')' : ''}}</div>
            <div class="{{=buttonclass(it.ismy)}}">{{=buttonname(it.ismy)}}</div>
        </div>
        <div class="row2">{{=guige(it)}}</div>
        <div class="row4">{{= (it.mark) ? '备注：'+it.mark : ''}}</div>
        <div class="row4">{{= (it.address) ? '用苗地：'+it.address : ''}}</div>
    ...
    <script type="text/javascript">
        ...
        $('#show').on('click','.edit',function(e){
            ...
            for (var i = 0; i < alldate.length; i++) {
                if((alldate[i].id == id)&&(alldate[i].orderid == orderid)){
                    json = alldate[i];
                    onedata = i;
                    $('#attributein').show();
        ...
        $('.add').click(function(){
            ...
            if($.trim($('#mark').val())!='') content.mark = $('#mark').val();
            ...
            if((content.count != null) && (content.name != null)){
                ...
                id = $('.add').data('id');
                $.getJSON('/com/order_tree_update.php',{userid:user.userid,id:id, data:JSON.stringify(content)},function(result){
                    if(result){
                        $('#attributein').hide();
