...
<body ontouchstart>
    ...
    <script id="dot_bid_order" type="text/x-dot-template">
        {{ for(var i in it) { }}
            <div class="{{=checkclass(it[i].state)}}" data-userid="{{=it[i].userid}}" data-state="{{=it[i].state}}" data-orderid="{{=it[i].orderid}}">
                <div class="tenderuserinfo">
                    <div class="bid_content_row1">
                        <div class="bid_content_name">{{=it[i].name}}{{=it[i].unit}}</div>
                        <div class="bid_content_price">{{=(it[i].price/100)}}元/{{=it[i].unit}}</div>
                        <div class="bid_content_result" style="color:{{= (it[i].state > 1) ? 'red' : '#888'}}">{{=witchsate(it[i].state)}}</div>
                    </div>
                    <div class="bid_content_row2">{{=attributes_show(it[i])}}</div>
                    <div class="bid_content_row2">{{=it[i].money ? '买家付款：'+it[i].money/100+'元' : ''}}</div>                     
                </div>
    ...
    <script type="text/html" id="tpl_tender">
        ...
        <script type="text/javascript">
            ...
            $('.bigimages').css('width',windowwidth);                    
            $('.bigimages').css('height',windowheight); 
            var timenow;                   
            loadscreenlist();
                ...
                if(state == 1){
                    $.getJSON('/com/bid_order_deal.php',{id:id,orderid:orderid,supplierid:supplierid},function(data){
                        if(data){
                            ...
                            that.attr('state','2');
                            that.next().remove();
                            that.parents('.showone').find('.showone_title').css('color','#E64340');
                        }
                    })
                }else if(state == 2){
                    var topay = confirm('确定要付定金吗？');
                    if(topay) window.location = "./goodspay.php?tree_order_id="+id+"&tender_userid="+user.userid+"&bid_userid="+supplierid+"&name="+name;
                }else if(state == 3){
                    var topay = confirm('确定要付尾款吗？');
                    if(topay) window.location = "./goodspay.php?tree_order_id="+id+"&tender_userid="+user.userid+"&bid_userid="+supplierid+"&name="+name;
                }else if(state == 4){
                    var topay = confirm('您的苗木款将支付给乙方，确定要收货？');
                    if(topay){
                        // $.getJSON('/com/payment.php',{tender_userid:user.userid,bid_userid:supplierid,tree_order_id:id},function(result){
                        //     if(result){
                        //         alert('收货成功！货款已转给乙方');
                        //     }
                        // })
            ...
            <div class="page__bd" style="height: 100%;">
                <div class="weui-tab">
                    <div class="headtitle">
                        ...
                        <div class="topbutton checked undone">
                            待选标
                        </div>
                        <div class="topbutton successbid">
                            已中标
                        </div>
