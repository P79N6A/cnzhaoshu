...
<head>
    ...
    <style type="text/css">
        ...
        .button_paydown,.button_payfull,.button_applyrefund,.button_detail,.button_evaluate,.button_receipt,.button_refunding,.button_notreceipt,.button_handing,.button_over{
            float: right;
            display: inline-block;
            padding: .15em .4em;
            min-width: 8px;
...
<body ontouchstart>
        ...
        {{ for(var i in it) { }}
            <div class="sell_onedatainfo" userid="{{=it[i].userid}}" order_oneid="{{=it[i].id}}">
                ...
                <div>
                    ...
                    {{=(it[i].deposit_switch == 1) ? '<div class="button_evaluate">申请定金</div>' : ''}}
                    {{=(it[i].state == 4) ? '<div class="button_over">交易完成</div>' : ''}}
    ...
    <script type="text/html" id="tpl_managerbuy">
        ...
        <script type="text/javascript">
            var timearray = ['payment_time','refund_time','order_switch_time','button_receipt_time','deposit_refund_time','fullamount_time','deposit_time','orderstart_time'];
            ...
            $('#orderinfo').on('click','.button_retreat',function(){
                ...
                $.getJSON('/wxpay/example/search_order_oneretreat.php', {userid:user.userid,id:order_oneid,}, function(result) {
                    if(result){
                        ...
                        $('#orderinfo').html(dot_onedatasell(sell_unsell_finsheddata));
                    }                
                });
            })
