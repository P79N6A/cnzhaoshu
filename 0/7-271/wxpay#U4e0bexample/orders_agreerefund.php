...
if($data && $wechatid){
    ...
    if($order['return_code'] == "SUCCESS" && $order['result_code'] == "SUCCESS"){
        ...
        $sql = 'update orders set refund_time=? , refund_mount=? , order_switch=4 ,refund_no=?, state=4 , deposit_switch=2 where serial_number=?';
        $result = $db->prepare_exec( $sql, array($payment_time,$money,$payment_no,$serial_number) );
        ...
        $result = $db->prepare_exec( $sql, array($data['bid_userid'],$data['deposit']) );
