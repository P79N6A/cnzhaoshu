...
<body ontouchstart>
        ...
        {{ for(var i in it) { }}
            <div class="bids" data-userid="{{=it[i].userid}}" check="{{= (it[i].state==2) ? true : false}}">
                ...
                <div class="button" state="{{=it[i].state}}">
                    ...
                    {{= (it[i].state == 4) ? '<div class="notreceipt">未到收货</div>' : ''}}
                    {{= (it[i].state == 3) ? '<div class="refund">申请退款</div>' : ''}}
    ...
    <script type="text/html" id="tpl_bidhistory">
        ...
        <script type="text/javascript">            
            var timearray = ['payment_time','refund_time','negotiation_time','order_switch_time','receipt_time','fullamount_time','deposit_time','orderstart_time'];
            var timeimpact = [' ','refund_mount',' ',' ',' ','fullamount','deposit',' '];
            var timename = ['完成交易','退还定金','平台正在进行协商','申请退还定金：不处理五天后自动退还','确认收货：资金五天后到账','付全款','付定金','下订单'];
            loadsuccessbidorder();
            var overbidinfo = [];
