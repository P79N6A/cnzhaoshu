...
function showtree (i) {
	...
	// var renzheng = tree.state==2 ? '<a class="renzheng" href="renzheng.php">已认证</a>' : '';
	var renzheng = tree.state==2 
				   ? ($.inArray(tree.userrole,[101,9])>=0
				   	  ? '<span class="qjd">旗舰店</span>'
				   	  : '<span class="renzheng">已认证</span>')
...
function addMyTree(i){
	...
	$.post('com/addmytree.php?userid='+user.userid+'&treeid='+tree.treeid+isOwn ,function(){
    	alert('加入成功！');	
	});
}
...
$(function () {
	...
	if (navigator.userAgent.indexOf("MicroMessenger") > 0) {
		...
		loadJS('/js/wechat_test.js?t=201702102');
	} 
});
