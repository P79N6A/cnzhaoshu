...
<script type="text/javascript">
	...
	// 数据
		...
		function loadprojectlist(way){
			$.getJSON('/com/pcsearch_map_list.php',{userid:user.userid},function(result){
		...
		function loadtreelist(a,way){
			$.getJSON('/com/pcsearch_map_treelist.php',{userid:user.userid,id:a},function(result){
	...
	// 进度
		...
		function lookthisrecord(id,a){
			hasthisrecord = true;
			photo_isend = true;
			$.getJSON('/com/pcsearch_map_order_record.php',{id:id},function(json){
				if(json){
					thisrecord = json;
		...
		function lookthisrecords(id,a,b){
			...
			$.getJSON('/com/pcsearch_map_order_records.php',{id:id,limit:limit,begin:a,end:b,userid:user.userid},function(json){
				if(json){
		...
		function loadallimage(state,a,b,c){
			...
			photo_isloading = true;
			var limit = $('#listimage .photobox').length + ',20';
			$.getJSON('/com/pcsearch_time_images.php', {userid: user.userid,limit:limit,begin:a,end:b,id:c}, function(json) {
	...
	// 地图
		...
		function loadmapdata(a){
	    	$.getJSON('/com/pcsearch_map_recordlist.php', {userid: user.userid}, function(json) {
	    		if(json){
	    			map_data = json;
	    			if(a){
	    				chooseproject(a);
        ...
        function lookaddress(gpsx,gpsy,name){
			window.open('http://apis.map.qq.com/uri/v1/marker?marker=coord:'+gpsx+','+gpsy+';title:'+name+'&amp;key=CNPBZ-SM4WD-UHP4G-PSOWR-Z7ZVO-YKFJU&amp;referer=zhaoshu');   
		}

        // 加载正常图片页面样式
        function natrue(){
            $('#jcopeLightBox').css({"height":height+'px',"width":width+'px'});
            $('.lb_wrapper').css('width',(width-400)+'px');
            $('.lb_body').css('height',(height-130)+'px');
            $('#lbSidebar').css('left',(width-400)+'px');
            ...
            for (var i = 0; i < smallnow.length; i++) {
                if(smallnow[i].photo == now){
                    ...
                    if(smallnow[i].photogps){
                        ...
                        setMap(xy[0],xy[1]); 
                        $('#image_in_map').attr('onclick','lookaddress('+xy[0]+','+xy[1]+',\''+smallnow[i]['tree_name']+' '+smallnow[i]['tree_attribute']+'\')');
                    }else{
                        setMap(39.909604,116.397228);
                        $('#image_in_map').attr('onclick','lookaddress(39.909604,116.397228,\''+smallnow[i]['tree_name']+' '+smallnow[i]['tree_attribute']+'\')');
                    }
                }
            }
...
<script type="text/html" id="tpl_statistics">
    ...
    <script type="text/javascript">
		...
		function loadstatistics(){
			$.getJSON('/com/pcsearch_map_recordstate.php', {userid: user.userid}, function(json) {
				if(json){
					statisticdata = json;
					dealstatistics(statisticdata);
				}else{
    ...
    <script type="text/javascript">
    	...
    	$('.map_submit').click(function () {
    	    ...
    	    if(nowlat){
    		    ...
    		    $.get('/com/map_user_insert.php',{userid:1,name:projectname,x:nowlat,y:nowlag,address:addressdeteil},function(result){
    		    	if(result){
    		    		result = result.split(';');
    		    		var tempdata = [];
    		    		tempdata[0] = {};
...
<script type="text/html" id="tpl_viewbigimages">
    ...
    <script type="text/javascript">
        ...
        $('#tabbar').hide();
        getsmallimage(targrphotoname);
        natrue();
        $('#image_in_map').css('height',(height-292)+'px');
        // 初始化小地图
        var map = new qq.maps.Map($('#image_in_map')[0], {disableDefaultUI: true, zoom: 12});
        removeMapVersion('image_in_map');
        $('<img>').addClass('middleControl')
                  .attr('src','http://www.yangshuwang.com/img/poi.png')
