...
<title>我的订单</title>
    <style type="text/css">
        ...
        .paydown,.payfull,.applyrefund,.detail,.evaluate,.receipt{
            float: right;
...
<body>
        ...
        {{ for(var i in it) { }}
            <div class="onedatainfo" treeuserid="{{=it[i].treeuserid}}" order_oneid="{{=it[i].id}}">
                ...
                <div>
                    ...
                    <div class="receipt {{=(it[i].state == 2) ? '' : 'hide'}}">确认收货</div>
                    <div class="applyrefund {{=((it[i].deposit_switch == 1) && (it[i].order_switch < 3)) ? '' : 'hide'}}">{{=(it[i].order_switch == 2) ? '正在退定金' : '申请退定金'}}</div>
                    <div class="payfull {{=(it[i].state < 2) ? '' : 'hide'}}">付全款</div>
    ...
    <script type="text/javascript">
        ...
        $('#orderinfo').on('click','.receipt',function(){
            ...
            if(!sure) return;
            var order_oneid = that.parents('.onedatainfo').attr('order_oneid');
            $.get('/com/order_one_receipt.php',{id: order_oneid},function(result){
                if(result){
        ...
        $('.upload').click(function(){
            ...
            for (var i = 0; i < alldata.length; i++) {
                if(order_oneid == alldata[i].id){
                    var treeuserid = alldata[i].treeuserid;
                    var name = alldata[i].name;
                    var number = alldata[i].number;                    
                }                
            }
            $('.alertstop').hide();
            $('#remark').val('');
            $.get('/com/order_one_stop.php',{id: order_oneid,treeuserid:treeuserid,name:name,remark:remark,number:number},function(result){
                if(result){
