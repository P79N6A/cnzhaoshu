...
<body>
    <div class="title">
        <img style="float: left;margin-left: 0;" src="./img/back.png" alt="">
    ...
    <div class="weui-cells">
        <div class="weui-cell weui-cells_checkbox">
            ...
            <input id="peoples" class="weui-input" type="number" max="50" min="1" placeholder="请输入负责人姓名">
        </div>
        <div class="weui-cell weui-cells_checkbox">
            <label class="weui-label" data-id="8">联系电话</label>
            <input id="peoples" class="weui-input" type="number" max="50" min="1" placeholder="请输入负责人手机">
        </div>
    </div>
...
// 检查权限，获取当前养护项目
function checkArea(maporderid) {
    record = null;
    $.getJSON('com/record/check.php', {maporderid:maporderid, userid:user.userid}, function(data) {
        console.log(data);

        if (data) {
            var tree = data.tree;
            $('#treename').html(tree.name);
