...
if($way == 1){
	...
	$result = $db->prepare_insert($sql,array($userid,100*$money,2));
}elseif($way == 2){
	...
	$sql = 'insert into recharge_bill(userid,money,way) values(?,?,?)';
	$result = $db->prepare_insert($sql,array($userid,100*$money,3));
}
...
$keyword2 = ($way == 1) ? '买家已付定金'.$money.'元' :'买家已付全款'.$money.'元';
