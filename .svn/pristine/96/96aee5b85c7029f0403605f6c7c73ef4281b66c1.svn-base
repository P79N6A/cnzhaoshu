...
<body>
    <div class="title">
        ...
        <img id="myphoto" src="./images/ico4_1.png" alt="">
    </div>
...
var map_order_id = urlRequest('id');
...
$("#workitems input").on('click', function() { 
    ...
    if (!record || record.type!=worktype) {
        checkRecord(map_order_id, worktype);
    }
}); 
...
function deleteRecord() {
    $.post('com/map_recorddelete.php', {id: map_order_id,userid:user.userid});
    $('#uploaderFiles').html('');
    record = null;  
    $galleryImg.attr("style", '');   
}
    ...
    if (uploadIndex < $uploaderInput[0].files.length) {
        var file = $uploaderInput[0].files[uploadIndex++];
            lrz(file, {
                ...
                rst.formData.append('map_order_id', map_order_id);
...
$(function(){
    ...
    setTimeout(function() {
        ...
        $('#myphoto').on('click',function(){
            window.location.href = "./map_uploadmy.php";
        })
        // 隐藏对话框
