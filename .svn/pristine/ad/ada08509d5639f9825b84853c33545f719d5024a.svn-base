			...
			if (zip_entry_open($resource,$dir_resource)) {
				...
				if(!is_dir($file_name)){
					...
					if(!$PHPReader->canRead($excelname_path)){
						...
						$sql = 'insert into lmzm_user(company_name,area,user_name,phone,excel) values(?,?,?,?,?)';
						$lmzm_userid = $db->prepare_insert($sql,array($companyname,$companyarea,$companyusername,$companyphone,$excelname));
							...
							if($datas){
							    for ($i=0; $i < count($datas); $i++) { 
							        ...
							        foreach ($datas[$i] as $key => $value) {
							        	if($key != 'tree_name') $value = floatval($value);
							            if($key != 'price'){
