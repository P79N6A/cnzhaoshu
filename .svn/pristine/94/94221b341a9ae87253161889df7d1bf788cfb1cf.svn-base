...
class Wechat
{
	...
	public function getwxconfig($str='')
	{	
       	$timestamp = time();
       	$access_token = $this->access_token;
       	$nonceStr = 'RC5kX0jnNprdNis4';
		$tmpArr = array($access_token, $timestamp, $nonceStr,$str);
		sort($tmpArr, SORT_STRING);
		$tmpStr = implode( $tmpArr );
		$tmpStr = sha1( $tmpStr );
		...
		$config['signature'] = $tmpStr;
		$config['nonceStr'] = $nonceStr;
		$config['jsApiList'] = ['chooseImage','previewImage','uploadImage','downloadImage'];
		return $config;
	}
