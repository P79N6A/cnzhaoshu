...
<body>
	    ...
	    {{ for(var i in it) { }}
	        <div class="onedata" data-id="{{=it[i].id}}" data-orderid="{{=it[i].orderid}}" data-username="{{=it[i].username}}">
                ...
                <div class="row1">
                    ...
                    <div class="{{=it[i].ismy ? 'onedatainfo_button button1' : 'onedatainfo_button button2'}}">{{=it[i].ismy ? '已报价' : '报价'}}</div>
                </div>
	...
	<script type="text/javascript">
        ...
        function loadallorder_index(){
            ...
            $.getJSON('/com/search_tree_order.php',{limit:limit,userid:user.userid,name:name},function(json){
                if(json){
        ...
        function getImageUrl(){
            return 'http://test.cnzhaoshu.com/img/hall.jpg';
        }

        function getLink(){
            return 'http://test.cnzhaoshu.com/tenderall.php';
        } 

        function getDescription () {
            $.getJSON('/com/seach_alltendernumber.php',function(result){
                return '共有'+result+'条真实采购！';
            })            
        }
