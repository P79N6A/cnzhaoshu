        ...
        <script type="text/javascript">
            ...
            $('#screenlist').on('click','.besupplier',function(){
                ...
                if(state == 1){
                    ...
                    if(topay){
                        $.getJSON('/wxpay/example/pay.php',{tender_userid:user.userid,bid_userid:supplierid,tree_order_id:id},function(result){
                            if(result){
                                if(result.return_code == "SUCCESS" && result.result_code == "SUCCESS"){
                                    that.html('已收货');
                                    alert('收货成功！货款已转给乙方');
                                }else if(result.V2_ACCOUNT_SIMPLE_BAN){
                                    alert('对方微信未实名认证,请联系管理员！');
                                }else{
                                    alert('付款失败！');
                                }
                            }
