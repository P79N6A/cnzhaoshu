...
<body>
	...
	<script type="text/javascript">
		...
		$('.to_pay').click(function(){
			...
			// 发送消息
			$.getJSON('/com/tender_to_pay.php',{userid:orderinfo.userid,companyid:JSON.stringify(allcompany),orderid:orderid,to_tender:JSON.stringify(to_tender)},function(data){
				if(data){
					if(data == '-1'){
						alert('不可多次发布招标信息!');
					}else{
