...
<script type="text/javascript">
	...
	var ordertree_qrcodeid = urlRequest("id");
	if(ordertree_qrcodeid){
		// 买方信息
		photolistbuy = null;
		// 买方项目
		projectlistbuy = null;
		...
		$.getJSON('/com/search_map_ordertreeinfo.php', {qrcodeid: ordertree_qrcodeid}, function(json) {
			if(json){
				photolistbuy = json.mapphoto;
				photoallbuy = json.photo;
				changeover('home');
			}
		});
	}
	...
	function writeinfo(data){
		var str = '<div class="record_info"><div class="recordview"><div class="recordview_row">项目名称：</div><div class="recordview_row1" id="">'+data.projectname+'</div></div><div class="recordview"><div class="recordview_row">用苗地址：</div><div class="recordview_row1" id="">'+data.address+'</div></div><div class="recordview"><div class="recordview_row">苗木信息：</div><div class="recordview_row1" id="">'+data.tree_name+' '+data.number+data.unit+' '+data.attribute+'</div></div><div class="recordview"><div class="recordview_row">供&nbsp;&nbsp;应&nbsp;商：</div><div class="recordview_row1" id="">'+data.username+' '+'<a href="tel:'+data.phone+'">'+data.phone+'</a>'+'</div></div><div class="recordview"><div class="recordview_row">进&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;度：</div><div class="recordview_row1" id="">'+data.active+'</div></div><div class="recordview"><div class="recordview_row">时&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;间：</div><div class="recordview_row1" id="">'+data.time+'</div></div></div>';
		var photos= data.photo.split(';');
		for (var i = 0; i < photos.length; i++) {
			str += '<img src="/photos/o/'+photos[i]+'.jpg" onclick="window.pageManager.back()">';
		}
		return str;
	}
	function loadselect(){
    ...
    <script type="text/javascript">
    	...
    	$('#imagetip').on('click','li',function(){
    		if(ordertree_qrcodeid){
				...
				$('#listimage').html(loadstatisticphoto(photoallbuy));
				window.pageManager.go('projectimage');	
    		}else{
...
<script type="text/html" id="tpl_showallproject">
    <div class="page" id="mysearch">
		...
		<div class="qrcodeimages">
			<p class="qrcodeimage_font">将此二维码发送给您想要授权的上传用户</p>
    ...
    <script type="text/javascript">
		...
		$('#right_tree').on('click','.treelist_row6',function(){
			var qxqrcode = $(this).parent().attr('qxqrcode');
			$('#qrcodeimages3').attr('src','./maporderqxqrcode/'+qxqrcode+'.png');
			$('.qrcodeimage_font').html('将二维码发送给想要授权的上传用户');
		...
		$('#mysearch').on('click','.createnew',function(){
			if(ordertree_qrcodeid){
				window.pageManager.go('profile');
			}else{
				window.pageManager.go('createnew');
			}
		})
		// 看单个苗木
...
<script type="text/html" id="tpl_profile">
    ...
    <script type="text/javascript">
    	...
    	if(ordertree_qrcodeid){
    		$('.mangerlist').hide();
    		$('.relogin').show();
    		$('#selfinfo_headimg').attr('src','headimg/96/0.jpg');
	    	$('#selfinfo_name').html('请先登录');
    	}else{
    		$('.relogin').hide();
...
<script type="text/html" id="tpl_mangerproject">
    ...
    <script type="text/javascript">
    	$('#projects').css('height',(height-50)+'px');
    	var showdata = [];
    	if((projectlistbuy != null) && (projectlistbuy.length)){
	    	for (var i = 0; i < projectlistbuy.length; i++) {
    	...
    	$('#projects').on('click','.myprojectname_image',function(){
    		...
    		$('.qrcodeimage_font').html('将'+name+'二维码发送给卖家即可生成苗木监管');
    	...
    	$('#projects').on('click','.myprojectname_share',function(){
    		...
    		$('.qrcodeimage_font').html('扫描二维码加入项目监管');
    		$('#mangerprojectbox .qrcodeimages').show();
    	})
