...

{
    ...
    //查看当前项目下的手机号是否有userid
    public function is_null_userid($tell = "")
    {

        $is_null_userid_sql = "select userid from contract_info  where tel=$tell";

        $userid = $this->db->query($is_null_userid_sql)[0]['userid'];
//        var_dump($userid);die;
        return $userid;
    }
...
//            计算所有项目的百分比
    public function tender_tree($page = "", $partya_company_name = "", $tel = "", $status = "", $project_name = "", $audit_status = "")
    {
        ...
        for ($ec = 0; $ec < count($dat); $ec++) {

            $dat[$ec]['isnull_user'] = $this->is_null_userid($dat[$ec]['company_info']['tel']);

            if ($dat[$ec]['isnull_user'] == "") {

                $dat[$ec]['isnull_user'] = 0;

            } else {

                $dat[$ec]['isnull_user'] = 1;
