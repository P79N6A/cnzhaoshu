...
$sql = 'update orders set order_switch=3,order_switch_remark=?,order_switch_time=?,negotiation_time=? where tree_order_id=? and bid_userid=?';
$result = $db->prepare_exec($sql,array($remark,$time,$time,$tree_order_id,$bid_userid));
