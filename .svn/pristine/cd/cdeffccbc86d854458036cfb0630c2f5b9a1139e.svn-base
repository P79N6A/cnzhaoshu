...
ignore_user_abort();
$oldtime=60;
$turl="http://cnzhaoshu.com/wxpay/example/order_one_deposittimer.php";

$run = include '../../com/order_onedeposit_timer1.php';
...
if(!$run) die;
    ...
    $sql = 'select serial_number,treeuserid,userid,deposit from order_one where deposit_switch=1 and (order_switch=1 or order_switch=4) and deposit_time < \''.$time.'\'';
    ...
    if(count($unfinshed)){
        for($i=0; $i < count($unfinshed); $i++) {       
            ...
            if($order['return_code'] == "SUCCESS" && $order['result_code'] == "SUCCESS"){
                ...
                $sql = 'update order_one set deposit_refund_time=?,deposit_switch=2 where serial_number=?';
