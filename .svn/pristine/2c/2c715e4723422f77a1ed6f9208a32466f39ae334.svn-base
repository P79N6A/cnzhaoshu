...
class Wechat
{
	...
	public function getSnsapi($code){
		$snsapi_Access_token = file_get_contents ( $snsapi_Access_token );
		if($snsapi_Access_token){
			$snsapi_Access_token = json_decode($snsapi_Access_token,true);
			$Access_token = $snsapi_Access_token['access_token'];

			$result = $this->http_get(self::OAUTH_AUTH_URL.self::OAUTH_TOKEN_URL.'='.$Access_token.'&openid='.$openid);

			$result = json_decode($result,true);
			if($result['errmsg'] == 'ok'){
				$this->getSnsapi_userinfo($Access_token);
			}else{
	...
	public function getAccess_token($code){		
		...
		if ($result['errcode'] != 40029){
			file_put_contents( $snsapi_Access_token, json_encode($result) );

			$this->getSnsapi_userinfo($result['access_token'],$result['openid']);
		}else{
			$this->getAccess_token($code);
		}
	}

	public function getSnsapi_userinfo($access_token,$openid){		
		$userinfo = $this->http_get(self::OAUTH_USERINFO_URL.self::OAUTH_TOKEN_URL.'='.$access_token.'&openid='.$openid.'&lang=zh_CN');
