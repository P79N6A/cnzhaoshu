...
if($id && $remark){
	...
	$sql = 'select a.treeuserid,a.userid,a.serial_number,a.number,b.* from (select * from order_one where id=?) a left join (select * from tree) b on a.treeuserid=b.userid and a.treeid=b.treeid';
	...
	if($result){
		...
		$user = user::getUserByUserId($orderinfo['userid']);
