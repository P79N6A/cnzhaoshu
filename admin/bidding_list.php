...
//
    ...
    if (empty($nursery_info)) {
        ...
        $sql = "select  count(tree_order_id) as num from new_tree_order  where  $where and unix_timestamp(Up_time) >= '".$new_time."' and tree_order_id not in (select tender_tree_id from tender_order where tender_user_id = '" . $user . "') ";
        ...
        $sqli = "select * from new_tree_order where $where and unix_timestamp(Up_time) >= '".$new_time."'  and tree_order_id not in (select tender_tree_id from tender_order where tender_user_id = '" . $user . "') order by Up_time desc limit $start,$page_num  ";
