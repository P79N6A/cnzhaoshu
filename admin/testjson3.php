...
//        $content = '招标支付1' . count($showdata) . '元';

//        imagettftext($image, 16, 325, 313, 69, $colorred, $font, $content);
...
//        var_dump($image);die;
        ...
        imagecopyresampled($image, $qrcode, 150, $height, 0, 0, 200, 200, 500, 500);
        ...
        for ($i = 0; $i < count($showdata); $i++) {
            ...
            $content = $showdata[$i]['tree_name'];

            imagettftext($image, 17, 0, 50, $height, $color, $font, $content);

            $content = $showdata[$i]['tree_num'] . '(' . $showdata[$i]['company'] . ')';
