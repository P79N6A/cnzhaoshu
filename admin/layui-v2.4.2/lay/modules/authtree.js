...
* @Last Modified time: 2018-08-02 17:42:45
*/
// 节点树
	...
	obj = {
		...
		renderAuth: function(tree, dept, opt){
			...
			layui.each(tree, function(index, item){
				...
				str += '<div><div class="auth-status"> '+(hasChild?'<i class="layui-icon auth-icon auth-leaf '+(openall?'active':'')+'" style="cursor:pointer;">'+(openall?'&#xe625;':'&#xe623;')+'</i>':'<i class="layui-icon auth-leaf" style="opacity:0;">&#xe626;</i>')+(dept > 0 ? '<span></span>':'')+'<input type="checkbox" name="'+inputname+'" title="'+item.name+'" value="'+item.id+'" lay-skin="primary" lay-filter="'+layfilter+'" '+(item.checked?'checked="checked"':'')+'  '+(item.disabled?'disabled="disabled"':'')+'> </div><div class="auth-child" style="'+(openall?'':'display:none;')+'padding-left:40px;"> '+append+'</div></div>'
			});
			str += '</div>';
			return str;
		},
