...
<body>
	...
	<script type="text/javascript">
		...
		function loadqualifiedcompany(){
			$.getJSON('/com/search_tendering_order.php',{orderid:orderid,userid:user.userid},function(json){
				if(json){
					...
					if(json['orderinfo'].pay_state) $('.foot').hide();
				}
			})
		}
		...
		function showall_company(json){
			...
			for (var i = 0; i < json.length; i++) {
				...
				if(json[i].data){
					...
					tree_allcompany[i].count  = tree_info.data.length;
					...
					for (var j = 0; j < tree_allinfo.length; j++) {
						...
						if(!allcompany.length){
							allcompany[allcompany.length] = tree_allinfo[j].userid;
							continue;
						}
						for (var m = 0; m < allcompany.length; m++) {
							if(allcompany[m] == tree_allinfo[j].userid){
								have = false;
							}
						}
						if(have) allcompany[allcompany.length] = tree_allinfo[j].userid;
					}
		...
		$('.to_pay').click(function(){
			...
			// 付款金额 同时发送消息
			alert(user.userid,allcompany,orderid,JSON.stringify(to_tender));
			$.post('/com/tender_to_pay.php',{userid:user.userid,companyid:JSON.stringify(allcompany),orderid:orderid,to_tender:JSON.stringify(to_tender)},function(data){
				if(data){
					if(data == -1){
						alert('操作失败!');
					}else if(data == -2){
						...
						window.location = './yusuanphone.php?show=1';
					}
				}
			})
		})
