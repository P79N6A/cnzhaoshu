...
class QrcodeImage
{
  public static function logo($img, $logo) 
  {
      ...
      $logo_size = $img_width/5;

      $xy = ($img_width - $logo_size) / 2;   // logo 居中布局的起始坐标

      // $image =imagecreatetruecolor($img_width, $img_width);
      // imagecopyresampled($image, $img, 0, 0, 0, 0, $img_width, $img_width, $img_width, $img_width);
      // unset($img);
      
      //重新组合图片并调整大小   
      imagecopyresampled($img, $logo, $xy, $xy, 0, 0, $logo_size, $logo_size, $logo_width, $logo_width);  
      // imagecopyresampled($image, $logo, $xy, $xy, 0, 0, $logo_size, $logo_size, $logo_width, $logo_width);  
