...
*/ 
class WaterMask{ 
private $waterType = 0; //水印类型：0为文字水印、1为图片水印 
private $pos = 0; //水印位置 
private $transparent = 80; //水印透明度 

private $waterStr = '找树网'; //水印文字 
private $waterStr2 = '13801305541'; //水印文字 
private $fontSize = 16; //文字字体大小 
private $fontSize0 = 16; //文字字体大小 
private $fontColor = array(255,255,255); //水印文字颜色（RGB） 
private $fontFile = '../font/simhei.ttf'; //字体文件 
...
function __construct($srcImg, $desImg, $waterStr, $fontSize) { //析构函数 
// $this->srcImg = file_exists($img) ? $img : die('"'.$img.'" 源文件不存在！'); 
	$this->srcImg = $srcImg;
	$this->desImg = $desImg;
	$this->waterStr2 = $waterStr;
	$this->fontSize0 = $fontSize;
	$this->output();
} 
...
private function waterpos() { //水印位置算法 
	switch ($this->pos) { 
		...
		default: //中下 
		$this->x = ($this->srcImg_info[0]-$this->waterImg_info[0])/2; 
		$this->y = $this->srcImg_info[1]-$this->waterImg_info[1]-12; 
		break 1; 
	} 
} 
...
private function waterstr() { 
	$this->fontSize = $this->fontSize0/2;
...
private function waterstr2() { 
	$this->fontSize = $this->fontSize0;

	$rect = imagettfbbox($this->fontSize,0,$this->fontFile,$this->waterStr2); 
	$w = abs($rect[2]-$rect[6]+10); 
	$h = abs($rect[3]-$rect[7]+2); 
	$fontHeight = $this->fontSize; 
	$this->water_im = imagecreatetruecolor($w, $h); 
	...
	imagefill($this->water_im,0,0,$white_alpha); 
	$color = imagecolorallocate($this->water_im,$this->fontColor[0],$this->fontColor[1],$this->fontColor[2]); 
	imagettftext($this->water_im,$this->fontSize,0,0,$this->fontSize,$color,$this->fontFile,$this->waterStr2); 
...
function output() { 
	...
	switch ($this->srcImg_info[2]) { 
		case 3: 
		imagepng($this->im,$this->desImg); 
		break 1; 
		case 2: 
		imagejpeg($this->im,$this->desImg); 
		break 1; 
		case 1: 
		imagegif($this->im,$this->desImg); 
		break 1; 
		default: 
		die('添加水印失败！'); 
		break; 
