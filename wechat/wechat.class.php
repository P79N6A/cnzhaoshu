...
class Wechat
{
	...
	private $user_token;
	private $partnerid;
	private $partnerkey = 'k39sof02idnx039a0eldi294jdi486hg';	                       
	private $paysignkey;
	private $postxml;
	private $_msg;
     ...
     * @param string $spbill_create_ip      用户终端ip 
     * @return string   XML 结构的字符串 
     */  

    public function pay($openid,$trade_no,$money,$desc,$spbill_create_ip){  
        $data = array(  
        ...
        //生成签名  
        $data['sign'] = self::getSigntopay($data);  
        //构造XML数据 
        $xmldata = self::arrayToXml($data); 

        $url = 'https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers';  
        //发送post请求  
        $res = self::http_post($url, $xmldata); 
       	var_dump(1,$data);
        if(!$res){  
    ...
    public function getSigntopay($arrdata){
    	foreach ($Obj as $k => $v){
	        $Parameters[$k] = $v;
	    }
	    //签名步骤一：按字典序排序参数
	...
	//  * 修改消息发送者
	//  */
	// public function setRevFrom($name) {
...
<Nonce><![CDATA[%s]]></Nonce>
	 ...
	 * @param int $expire 临时二维码有效期，最大为1800秒，30分钟
	...
	public function getQRCode($scene_id,$type=0,$expire=1800){
		...
		// echo self::API_URL_PREFIX.self::QRCODE_CREATE_URL.'access_token='.$this->access_token,self::json_encode($data);
