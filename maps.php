...
<script src="http://map.qq.com/api/js?v=2.exp&key=CNPBZ-SM4WD-UHP4G-PSOWR-Z7ZVO-YKFJU"></script>
...
<script type="text/javascript">
	...
	function lookaddress(gpsx,gpsy,name){
		var photodatas = escape(JSON.stringify(rimagedata));
		document.cookie="rimagedata="+photodatas;
	...
	// 进度
		function loadstatisticphoto(photo){
			...
			for (var i in photo) {
				...
				if(photos.search("/admin/upload") != -1){
					if(datetime(photo[i].time) == time){
						str += '<div class="photobox" onclick="lookphotoinfo('+photo[i].id+');"><div class="treephoto" style="background-image:url('+photos+')"><p>'+photo[i].tree_name+' '+photo[i].active+'<br>'+photo[i].tree_attribute+'</p></div></div>';
					}else{
			...
			$.getJSON('/com/search_map_order_records.php',{id:id,limit:limit,begin:a,end:b,userid:user.userid},function(json){
				if(json){
					...
					for (var i = 0; i < json.length; i++) {
						rimagedata[rimagedata.length] = json[i];
					}
	...
	// 地图
		...
		function getTooltip(data) {
		    ...
		    if (data.photo) {
				...
				if(photo.search("/admin/upload") != -1 ){
					result.image = photo;
				}else{
...
<script type="text/html" id="tpl_projectimage">
    ...
    <script type="text/javascript">
    	...
    	$_listimage.css('height',(height-92)+'px');
	   	...
	   	if(getcookie('rimagedata')){
			rimagedata = getcookie('rimagedata');
	    	rimagedata = JSON.parse(rimagedata);
	   		$listimage.html(loadstatisticphoto(rimagedata));
	   		document.cookie="rimagedata=";
	   	}else{
	    	if(hasthisrecord){
